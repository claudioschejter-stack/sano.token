    ...
    <article className="flex h-full flex-col rounded-xl border border-terminal-border bg-terminal-card p-4 shadow-[0_0_0_1px_rgba(31,41,55,0.5)] sm:p-5">
      <div className="flex flex-1 items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p className="min-h-[2.5rem] text-sm font-medium leading-snug text-terminal-muted">{label}</p>
          <p
            className={`mt-2 font-mono font-bold leading-tight tracking-tight ${valueClassName} text-[clamp(1.125rem,2.5vw,1.75rem)]`}
          >
            {value}
          </p>
          <p className="mt-2 min-h-[2rem] text-sm leading-snug text-terminal-muted">{hint}</p>
        </div>
