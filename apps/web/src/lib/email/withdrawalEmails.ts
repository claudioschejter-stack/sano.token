...
export async function sendWithdrawalConfirmedEmail(input: {
  ...
  const txUrl = basescanTxUrl(input.txHash);
  ...
  const bodyHtml = `
    ...
    ${renderEmailButton(txUrl, 'Ver transacción en Basescan')}
    <p style="color:#64748b;font-size:13px">Hash: ${input.txHash}</p>
  `;

  await sendTransactionalEmail({
    to: input.to,
    subject: `Retiro confirmado · USD ${amount}`,
    text: `Hola ${greetingName}, tu retiro de USD ${amount} fue enviado. Transacción: ${txUrl}`,
