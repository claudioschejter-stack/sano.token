...
import { safeLogId } from '../logging/safeLogValue';
...
export async function ensureBridgeOnboarding(input: {
  ...
  try {
    ...
    console.error('[bridgeCustomerService] ensure failed', safeLogId(input.userId), error);
...
export async function startBridgeOnboardingAfterKyc(userId: string): Promise<void> {
  ...
  await ensureBridgeOnboarding({
    ...
    console.error('[bridgeCustomerService] start after KYC failed', safeLogId(userId), error);
    return NOT_CONFIGURED;
  });
}
