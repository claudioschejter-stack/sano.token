...
describe('buildSettlementCharges', () => {
  it('sends investment plus coverage to treasury', () => {
    ...
    expect(charges.treasuryTransferUsd).toBe(20.25);
    expect(charges.networkFeeUsd).toBe(0.254253);
    expect(charges.payableUsdc).toBe(20.254253);
  });
  ...
  it('keeps the investment amount untouched', () => {
    ...
    expect(charges.coverageUsd).toBe(0.25);
  });
