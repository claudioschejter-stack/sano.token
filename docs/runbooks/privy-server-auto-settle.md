...
| Who pays gas for an investor purchase | The investor: their own USDC transfer (Privy paymaster) **plus** a flat `RWA_INVESTOR_GAS_COVERAGE_USD` (default 0.032 USDC) that travels with the payment to treasury to fund `setKyc` and the vault share delivery | Raise/lower it in Vercel. `0` disables the coverage and the platform absorbs those txs again. |
| `Watch crypto deposits (backup)` action red with `curl 500` | One sweep stage threw and the route returned an opaque `WATCH_SWEEP_FAILED` | The cron now runs each stage in isolation and answers `{ failures: [{ stage, error }], stages }`. Call it manually with the cron header and read `failures` to see which scan is broken (usually a misconfigured `BASE_RPC_URL`). |
| Want multisig **and** automatic whitelisting | The asset token is `Ownable`, so the owner can mint, pause and transfer ownership — handing that key to automation trades governance for convenience | Two-tier setup: the Safe stays token owner and `SanovaKycOperatorModule` lets the operator call **only** `setKyc`. `GET /api/admin/kyc-module-setup` shows the state; `POST { safeAddress, projectIds? }` deploys the module, enables it on the Safe, scopes it to the tokens and grants the operator. Then set `KYC_OPERATOR_MODULE_ADDRESS`. |
| No Privy wallet owns the token | The first RWA contracts are owned by the legacy deploy EOA (`0x7AC277Cd…`, an EOA with nonce 263) or by the legacy Safe (`0x5e7480c4…`), so `setKyc` cannot be signed through Privy | One-off: `POST /api/admin/transfer-token-ownership { "projectId": "…" }` signs `transferOwnership` with `TOKEN_DEPLOY_PRIVATE_KEY` and hands token + vault to the Privy operator wallet. Then set `RWA_OPERATOR_ADDRESS` to that wallet's address. Safe-owned contracts need the Safe flow instead. |
| `setKyc` cannot be signed | The asset token is `Ownable`: **only `owner()`** may call `setKyc`. If no Privy wallet is the owner, the server cannot whitelist at all. | `GET /api/admin/privy-wallets` lists the app's ethereum wallets with Base mainnet balances, each token's `owner()`, and `recommendedOperatorWalletId`. If no wallet matches the owner, `transferOwnership` to one of them first. |
| `insufficient funds` with a funded operator | Wrong chain or wrong signing wallet. `GET /api/admin/investor-allowlist?email=…` → `operator.signingChainId` must be **8453**, `operator.rpcChainId` must match it, and `operator.addressMatchesWalletId` must be `true`. | Set `TOKEN_DEPLOY_CHAIN_ID=8453`, `BASE_RPC_URL=https://mainnet.base.org`, and `PRIVY_OPERATOR_WALLET_ID` to `operator.suggestedWalletIdForAddress` (the wallet whose address is the token `owner()`). |
| `insufficient funds for transfer` when whitelisting | The RWA operator server wallet has no Base ETH for `setKyc` | `GET /api/admin/investor-allowlist?email=…` returns `operator.address` and `operator.ethBalance`. Send ~0.005 ETH there, or `GET /api/cron/fund-gas?to=<operator>`. Server-wallet sends also try Privy **app gas credits** first (`PRIVY_SPONSOR_SERVER_TX`, default on). |
| `ALLOWLIST_NOT_APPROVED` / `ONCHAIN_ALLOWLIST_NOT_APPROVED` on Pagar | The wallet is not whitelisted on the project asset token (`kycApproved(wallet) === false`) and/or has no `investorAllowlist` row. Settle blocks **before** taking USDC. | `GET /api/admin/investor-allowlist?email=…` lists the gaps; `POST` the same route whitelists on-chain + DB (needs `PRIVY_OPERATOR_WALLET_ID` + `RWA_OPERATOR_ADDRESS`). Reconciliation also attempts it for already-paid batches. |
