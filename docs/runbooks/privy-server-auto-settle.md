...
| Who pays gas for an investor purchase | The investor: their own USDC transfer (Privy paymaster) **plus** a flat `RWA_INVESTOR_GAS_COVERAGE_USD` (default 0.022 USDC) that travels with the payment to treasury to fund `setKyc` and the vault share delivery | Raise/lower it in Vercel. `0` disables the coverage and the platform absorbs those txs again. |
