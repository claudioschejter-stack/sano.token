...
contract SanovaAsyncVault is SanovaRwaVault {
        ...
        returns (uint256 requestId)
    {
        ...
        // `owner` is authorized above via _authorizeController (must be msg.sender or an
        // approved operator), so this can only move funds the caller is already entitled to
        // move, not an arbitrary third party's.
